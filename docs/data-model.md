...
| `discovery_runs` | one row per budgeted run: the locked budget JSON recorded at start, spend and per-source outcomes at finish, exhaustion stage, and (migration 0009) the lease owner and fence the run held, which is what makes an abandoned run provable | run_seq UNIQUE; status CHECK adds the terminal `interrupted` for runs whose process died, reconciled by `discover recover` or the next run start |
