...
| `discovery_runs` | one row per budgeted run: the locked budget JSON recorded at start (the lease TTL and renewal interval included), spend and per-source outcomes at finish, exhaustion stage, (migration 0009) the lease owner and fence the run held, which is what makes an abandoned run provable, and (migration 0011) `failure_json`, the diagnostic an aborted run persists: exception type and message plus the stage and source id when known | run_seq UNIQUE; status CHECK adds the terminal `interrupted` for runs whose process died, reconciled by `discover recover` or the next run start |
