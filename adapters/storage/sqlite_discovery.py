...
class SqliteDependencyEpochRepository(DependencyEpochRepository):
    ...
    def sync_families_fingerprint(self, fingerprint: str) -> int:
        ...
        The comparison lives in the UPDATE's WHERE clause rather than in a
        preceding SELECT because pysqlite starts its implicit transaction at
        the first WRITE: a separate SELECT would run outside it, letting two
        connections both read the old fingerprint and both bump. SQLite's
        `IS NOT` is the null-safe comparison, so a stored NULL still counts as
        a difference."""
        with self._conn:
            self._conn.execute(
                "UPDATE dependency_epoch SET epoch = epoch + 1,"
                ...
                " WHERE id = 1 AND families_fingerprint IS NOT ?",
